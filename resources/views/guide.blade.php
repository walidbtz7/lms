            ...
            @foreach($mytrails as $mytrail)
                ...
                <div class="grid-item card hoverable sticky-action">
                    ...
                    <div class="card-action">
                        <a class="btn btn-floating waves-effect waves-light blue tooltipped" data-position="bottom" data-tooltip="View Matter" href="{{ url('trails/'. $mytrail->trail_id) }}"><i class="material-icons">remove_red_eye</i></a>
                        <a class="btn btn-floating waves-effect waves-light red tooltipped modal-trigger" data-target="#remove{{$mytrail->trail_id}}" href="#modal2" data-position="bottom" data-tooltip="Remove from Matter"><i class="material-icons">remove</i></a>
                        <a class="btn btn-floating waves-effect waves-light black tooltipped" data-position="bottom" data-tooltip="Start Matter" href="{{ url('trails/start/'. $mytrail->trail_id) }}"><i class="material-icons">play_arrow</i></a>
                    </div>
                </div>
                ...
                <div class="modal" id="#remove{{$mytrail->trail_id}}">
                    <div class="modal-content">
                        ...
                        <p>
                            Do you want to remove the <span class="red-text">{{ $mytrail->title }}</span> from your list?
                        </p>
                        <small class="grey-text">Your progress data will always be saved :)</small>
                    </div>
            ...
            @if($mytrails->count() != 0)
                ...
                @foreach($diff as $trail) 
                    ...
                    <div class="grid-item card hoverable sticky-action">
                            ...
                            <div class="card-action">
                            @if (Auth::check())
                                <a class="btn btn-floating waves-effect waves-light blue tooltipped" data-position="bottom" data-tooltip="View Matter" href="{{ url('trails/'. $trail->id) }}"><i class="material-icons">remove_red_eye</i></a>
                                <a class="btn btn-floating waves-effect waves-light green tooltipped" data-position="bottom" data-tooltip="Add to Matter" href="{{ url('trails/add/'. $trail->id) }}"><i class="material-icons">add</i></a>
                            @else
                                <a class="btn btn-floating waves-effect waves-light blue tooltipped" data-position="bottom" data-tooltip="View Matter" href="{{ url('trails/'. $trail->id) }}"><i class="material-icons">remove_red_eye</i></a>
                            @endif
                            </div>
